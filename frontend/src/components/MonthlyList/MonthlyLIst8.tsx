...
const MonthlyList8: FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const responsePlus = await fetch(`http://3.38.102.194:8888/api/total/plus/${year}-08-01/${year}-08-31`);
        const plusData = await responsePlus.json();
        
        const responseMinus = await fetch(`http://3.38.102.194:8888/api/total/minus/${year}-08-01/${year}-08-31`);
  ...
  return (
    <button className={MonthlyBox4} onClick={handleClick}>
      <div className={alphabet}>
        <span>{`8월 (+${plusTotal})/(-${minusTotal}) 내역`}</span>
      </div>
    </button>
  );
};

export default MonthlyList8;