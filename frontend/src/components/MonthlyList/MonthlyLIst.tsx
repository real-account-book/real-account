...
import { alphabet, MonthlyBox1 } from './MonthlyList.css.ts';

const MonthlyList1: FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const responsePlus = await fetch(`http://3.38.102.194:8888/api/total/plus/${year}-01-01/${year}-01-31`);
        const plusData = await responsePlus.json();
        
        const responseMinus = await fetch(`http://3.38.102.194:8888/api/total/minus/${year}-01-01/${year}-01-31`);
  ...
  const handleClick = () => {
    if (year) {
      navigate(`/month/${year}01`);
    }
  };

  return (
    <button className={MonthlyBox1} onClick={handleClick}>
      <div className={alphabet}>
        <span>{`1월 (+${plusTotal})/(-${minusTotal}) 내역`}</span>
      </div>
    </button>
  );
};
