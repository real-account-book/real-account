...
const MonthlyList9: FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const responsePlus = await fetch(`http://3.38.102.194:8888/api/total/plus/${year}-09-01/${year}-09-30`);
        const plusData = await responsePlus.json();
        
        const responseMinus = await fetch(`http://3.38.102.194:8888/api/total/minus/${year}-09-01/${year}-09-30`);
  ...
  return (
    <button className={MonthlyBox5} onClick={handleClick}>
      <div className={alphabet}>
        <span>{`9월 (+${plusTotal})/(-${minusTotal}) 내역`}</span>
      </div>
    </button>
  );
};

export default MonthlyList9;